...
class AnthropicProvider(BaseProvider):
    ...
    capabilities = {
        "tools": True, "caching": False, "reasoning": False,
        "structured": False, "parallel_tools": True,
    }
    ...
    async def chat(self, messages, *, max_tokens=2048, temperature=0.7, model=None,
        ...
        if tools:
            body["tools"] = [{
                "name": (t if isinstance(t, dict) else t.model_dump())["name"],
                "description": (t if isinstance(t, dict) else t.model_dump()).get("description", ""),
                "input_schema": (t if isinstance(t, dict) else t.model_dump()).get("input_schema") or {"type": "object", "properties": {}},
            } for t in tools]
            if tool_choice is None or tool_choice == "none":
                # Anthropic doesn't natively support "none" tool_choice; omitting tool_choice is the equivalent
                pass
            elif isinstance(tool_choice, dict):
        ...
        async with httpx.AsyncClient(timeout=180) as c:
            ...
            if r.status_code != 200:
                raise ProviderError(f"anthropic HTTP {r.status_code}: {r.text[:300]}", status=r.status_code, retryable=(r.status_code not in (400, 401)))
            ...
            stop_norm = "tool_use" if stop_reason == "tool_use" else ("max_tokens" if stop_reason == "max_tokens" else "end_turn")
...
def model_capabilities(provider_name: str, model: str, default_caps: dict) -> dict:
    ...
    if provider_name == "anthropic":
        caps["reasoning"] = False # Not mapped yet
...
def build_providers(cache_store):
    ...
    if k := os.getenv("ANTHROPIC_API_KEY"):
        out["anthropic"] = AnthropicProvider(k, os.getenv("ANTHROPIC_MODEL", "claude-3-5-sonnet-20241022"))
...
ROUTER_DEFAULTS = {
    # llama3.1-8b deprecated 2026-05-27; zai-glm-4.7 is the confirmed working
    # model on this account as of May 2026.
...
def build_router_providers():
    ...
    we picked (Groq, NVIDIA, GitHub) all meter per-model, not per-key.

    Cerebras excluded: no model on this account reliably emits TINY/LARGE/HUGE
    in English; zai-glm-4.7 produces unparseable GLM-format replies every call,
    wasting ~700ms before falling back to Groq.
    """
    out = {}
    if k := os.getenv("GROQ_API_KEY"):
