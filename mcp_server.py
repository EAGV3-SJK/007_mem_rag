...
async def fetch_urls(urls: list[str]) -> list[dict]:
    """Fetch up to 3 URLs in parallel via crawl4ai. Prefer this over multiple fetch_url calls.
    Example: fetch_urls(["https://a.com", "https://b.com"])."""
    import asyncio
    batch = urls[:3]
    results = await asyncio.gather(*[_crawl4ai_fetch(u) for u in batch], return_exceptions=True)
    out = []
    for url, r in zip(batch, results):
        if isinstance(r, Exception):
            out.append({"url": url, "status": 0, "error": str(r), "text": ""})
        else:
            out.append({"url": url, **r})
    return out
...
# ── document indexing (Session 7 + bulk corpus) ─────────────────────────────

def _resolve_index_dir(path: str) -> Path:
    """Resolve a directory path for indexing.

    Accepts two forms:
    - ``corpus/...`` (or just ``corpus``) — resolved relative to PROJECT_ROOT,
      not the sandbox. Lets the agent index the pre-built AWS corpus without
      copying it into sandbox.
    - Anything else — resolved via ``_safe()`` (sandbox-sandboxed).

    Both forms are checked to stay within their respective roots.
    """
    norm = path.replace("\\", "/")
    if norm == "corpus" or norm.startswith("corpus/"):
...
@mcp.tool()
def index_directory(path: str = "corpus/aws", chunk_size: int = 400, overlap: int = 80) -> dict:
    """Bulk-index all .md and .txt files under a directory as searchable fact chunks.
    Accepts 'corpus/aws' (AWS markdown corpus under project root) or sandbox-relative paths.
    Use this once to seed the knowledge base before running search_knowledge.
    Example: index_directory("corpus/aws")."""
    try:
        base = _resolve_index_dir(path)
    except ValueError as e:
        return {"error": str(e)}
    if not base.exists():
        return {"error": f"Directory '{path}' not found at {base}"}
    if not base.is_dir():
        return {"error": f"'{path}' is not a directory"}

    run_id = f"idx-{datetime.now().strftime('%Y%m%d%H%M%S')}"
    total_chunks = 0
    files_ok = 0
    file_results: list[dict] = []

    all_files = sorted(base.rglob("*.md")) + sorted(base.rglob("*.txt"))
    for fp in all_files:
        try:
            text = fp.read_text(encoding="utf-8")
            if not text.strip():
                continue
            # Build a source label relative to PROJECT_ROOT so the descriptor
            # starts with "sandbox:corpus/..." making it recognisable to agents.
            try:
            ...
            chunks = _chunk_text(text, size=chunk_size, overlap=overlap)
            kw_base = [fp.stem] + fp.stem.replace("-", " ").split()
            for i, chunk in enumerate(chunks):
                preview = chunk[:120].replace("\n", " ")
                descriptor = f"[{source} chunk {i + 1}/{len(chunks)}] {preview}"
                _memory.add_fact(
                ...
                total_chunks += 1
            files_ok += 1
            file_results.append({"file": fp.name, "chunks": len(chunks)})
        except Exception as exc:
            file_results.append({"file": getattr(fp, "name", path), "error": str(exc)})
...
def search_knowledge(query: str, k: int = 5) -> list[dict]:
    """Hybrid vector+keyword search over indexed fact chunks (RRF-merged).
    Returns up to k ranked chunks with full content (up to 2000 chars each).
    Prefer this over re-fetching URLs or re-reading source files when Memory
    already contains indexed chunks — that is the whole point of indexing.
    Example: search_knowledge("Bedrock chunking strategies", 5)."""
