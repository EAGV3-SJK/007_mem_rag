...
from gateway import LLM, chat_with_fallback, embed as _gateway_embed, ensure_gateway
...
def _load() -> list[MemoryItem]:
    ...
    if not raw:
        return []
    return [MemoryItem.model_validate(r) for r in json.loads(raw)]
...
_RRF_K = 60  # standard RRF constant; higher = gentler rank penalty


def _rrf_merge(
    vec_hits: list[MemoryItem],
    kw_hits: list[MemoryItem],
    *,
    top_k: int,
) -> list[MemoryItem]:
    """Reciprocal Rank Fusion of two ranked lists.

    Each list contributes 1/(RRF_K + rank+1) to an item's score. Items that
    appear in both lists accumulate scores from both paths, naturally floating
    to the top. This lets lexical queries (q1–q3) score high via keyword rank
    and semantic queries (q4–q5) score high via vector rank — simultaneously.
    """
    ...
    return [by_id[iid] for iid, _ in ranked[:top_k]]
...
def _format_hits(
    ...
    *,
    ...
    """Render memory hits as a text block for Decision prompt consumption."""
    lines: list[str] = []
    total = 0
    for i, h in enumerate(hits[:max_hits]):
        content = h.value.get("chunk") or h.value.get("raw") or ""
        block = (
            f"[{i}] kind={h.kind} source={h.source!r}\n"
        ...
        lines.append(block)
        total += len(block)
    return "\n\n".join(lines) if lines else "(no memory hits)"
...
def read(
    ...
    *,
    ...
    """Hybrid search: vector + keyword merged via Reciprocal Rank Fusion.

    Both paths run every time. Lexical queries (q1–q3) score high via keyword
    overlap; semantic queries (q4–q5) score high via cosine similarity. Items
    appearing in both lists get a bonus from each, so overlap boosts precision.
    """
    vec_hits = _vector_search(query, kinds=kinds, top_k=top_k * 2)
    kw_hits = _keyword_search(query, history, kinds=kinds, top_k=top_k * 2)
    return _rrf_merge(vec_hits, kw_hits, top_k=top_k)
