...
async def execute(
    ...
    # fetch_url returns {"status":..., "text": "<page markdown>", ...}.
    # Unwrap the JSON so artifacts contain clean page text, not a JSON wrapper
    # that confuses LLMs trying to extract facts from the content.
    if tool_call.name == "fetch_url":
        try:
            d = json.loads(text)
            if isinstance(d, dict) and isinstance(d.get("text"), str) and len(d["text"]) > 100:
                text = d["text"]
